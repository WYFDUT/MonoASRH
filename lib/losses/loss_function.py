...
from lib.losses.uncertainty_loss import laplacian_aleatoric_uncertainty_loss,laplacian_aleatoric_uncertainty_loss_new,beta_laplacian_aleatoric_uncertainty_loss
...
class LSS_Loss(nn.Module):
    ...
    def compute_segmentation_loss(self, input, target):
        ...
        sgc_loss = self.compute_SGC_loss(input['heatmap'], K=50, thr=0.95, weight=0.01)
        loss = loss + sgc_loss
        self.stat['seg_loss'] = loss
        return loss


    def compute_SGC_loss(self, heatmap, K=50, thr=0.95, weight=0.01):
        heatmap_nms = _nms(heatmap)
        scores, inds, cls_ids, xs, ys = _topk(heatmap_nms, K=K)
        batch_size = scores.shape[0]
        mask = scores > thr
        scores = torch.masked_select(scores, mask)
        if scores.shape[0] == 0:
            SGC_loss = torch.tensor(0.0).to(heatmap.device)
            return SGC_loss
        scores = -torch.log(scores)
        SGC_loss = scores.sum() / batch_size * weight
        return SGC_loss
    ...
    def compute_bbox3d_loss(self, input, target, mask_type = 'mask_2d'):
        ...
        vis_depth_loss = laplacian_aleatoric_uncertainty_loss_new(vis_depth,
                                                        ...
                                                        vis_depth_uncer)  
        '''
        vis_depth_loss = beta_laplacian_aleatoric_uncertainty_loss(vis_depth,
                                                        vis_depth_target,
                                                        vis_depth_uncer)                                 
        '''
        device = vis_depth.device
        if self.epoch < 120: # 50
