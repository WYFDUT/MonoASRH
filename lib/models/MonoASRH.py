...
from lib.backbones.RTEncoder import HybridEncoder
from .attentive_normalization import AttnBatchNorm2d, AttnGroupNorm
from .deformconv import Deformconv, ScaleAwareDeformconv
...
class MonoASRH(nn.Module):
    def __init__(self, backbone='dla34', neck='DLAUp', downsample=4, mean_size=None):
        ...
        self.heatmap = nn.Sequential(Deformconv(channels[self.first_level], self.head_conv),
                                     nn.ReLU(inplace=True),
                                     nn.Conv2d(self.head_conv, 3, kernel_size=1, stride=1, padding=0, bias=True))
        self.offset_2d = nn.Sequential(Deformconv(channels[self.first_level], self.head_conv),
                                     nn.ReLU(inplace=True),
                                     nn.Conv2d(self.head_conv, 2, kernel_size=1, stride=1, padding=0, bias=True))
        self.size_2d = nn.Sequential(Deformconv(channels[self.first_level], self.head_conv),
                                     nn.ReLU(inplace=True),
                                     nn.Conv2d(self.head_conv, 2, kernel_size=1, stride=1, padding=0, bias=True))

        # use the attentive normalization.
        self.offset_3d_feat = ScaleAwareDeformconv(channels[self.first_level]+2+self.cls_num, self.head_conv)
        self.offset_3d = nn.Sequential(AttnGroupNorm(self.head_conv, self.head_conv//4, num_groups=self.head_conv//8, num_groups_attn=self.head_conv//8, eps=0.001),
                                     nn.LeakyReLU(inplace=True),nn.AdaptiveAvgPool2d(1),
                                     nn.Conv2d(self.head_conv, 2, kernel_size=1, stride=1, padding=0, bias=True))
        self.size_3d_feat = ScaleAwareDeformconv(channels[self.first_level]+2+self.cls_num, self.head_conv)
        self.size_3d = nn.Sequential(AttnGroupNorm(self.head_conv, self.head_conv//4, num_groups=self.head_conv//8, num_groups_attn=self.head_conv//8, eps=0.001),
                                     nn.ReLU(inplace=True),nn.AdaptiveAvgPool2d(1),
                                     nn.Conv2d(self.head_conv, 3, kernel_size=1, stride=1, padding=0, bias=True))
        self.heading_feat = ScaleAwareDeformconv(channels[self.first_level]+2+self.cls_num, self.head_conv)
        self.heading = nn.Sequential(AttnGroupNorm(self.head_conv, self.head_conv//4, num_groups=self.head_conv//8, num_groups_attn=self.head_conv//8, eps=0.001),
                                     nn.ReLU(inplace=True),nn.AdaptiveAvgPool2d(1),
                                     nn.Conv2d(self.head_conv, 24, kernel_size=1, stride=1, padding=0, bias=True))

        # self.vis_depth_feat = DepthScaleAwareDeformconv(channels[self.first_level]+2+self.cls_num, self.head_conv)
        self.vis_depth_feat = ScaleAwareDeformconv(channels[self.first_level]+2+self.cls_num, self.head_conv)
        self.vis_depth = nn.Sequential(nn.LeakyReLU(inplace=True),
                                       nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
        
        # self.vis_depth_uncer_feat = DepthScaleAwareDeformconv(channels[self.first_level]+2+self.cls_num, self.head_conv)
        self.vis_depth_uncer_feat = ScaleAwareDeformconv(channels[self.first_level]+2+self.cls_num, self.head_conv)
        self.vis_depth_uncer = nn.Sequential(nn.LeakyReLU(inplace=True),
                                             nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))

        # self.attention_feat = DepthScaleAwareDeformconv(channels[self.first_level]+2+self.cls_num, self.head_conv)
        self.attention_feat = ScaleAwareDeformconv(channels[self.first_level]+2+self.cls_num, self.head_conv)
        self.attention = nn.Sequential(      nn.LeakyReLU(inplace=True),
                                             nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
        ...
        # init 3d heads
        self.offset_3d_feat.apply(weights_init_xavier)
        self.size_3d_feat.apply(weights_init_xavier)
        self.heading_feat.apply(weights_init_xavier)
        self.vis_depth_feat.apply(weights_init_xavier)
        self.vis_depth_uncer_feat.apply(weights_init_xavier)
        self.attention_feat.apply(weights_init_xavier)
    ...
    def get_roi_feat_by_mask(self,feat,box2d_maps,inds,mask,calibs,coord_ranges,cls_ids):
        ...
        if num_masked_bin!=0:
            ...
            size3d_offset_feat = self.size_3d_feat(roi_feature_masked, scale_box2d_masked)
            size3d_offset = self.size_3d(size3d_offset_feat)[:,:,0,0]
            ...
            vis_depth_feat = self.vis_depth_feat(roi_feature_masked, scale_box2d_masked)
            vis_depth = self.vis_depth(vis_depth_feat).squeeze(1)
            attention_map_feat = self.attention_feat(roi_feature_masked, scale_box2d_masked)
            attention_map = self.attention(attention_map_feat).squeeze(1)
            ...
            vis_depth_uncer_feat = self.vis_depth_uncer_feat(roi_feature_masked, scale_box2d_masked)
            vis_depth_uncer = self.vis_depth_uncer(vis_depth_uncer_feat)[:, 0, :, :]
            res['train_tag'] = torch.ones(num_masked_bin).type(torch.bool).to(device_id)
            heading_feat = self.heading_feat(roi_feature_masked, scale_box2d_masked)
            res['heading'] = self.heading(heading_feat)[:,:,0,0]

            res['vis_depth'] = vis_depth
            res['vis_depth_uncer'] = vis_depth_uncer
            offset_3d_feat = self.offset_3d_feat(roi_feature_masked, scale_box2d_masked)
            res['offset_3d'] = self.offset_3d(offset_3d_feat)[:,:,0,0]
